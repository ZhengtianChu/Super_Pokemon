...
void SaveState::Handle(Context* c)
{
	cout << "save file state" << endl;
}

SaveState::SaveState(Scyzc3Engine* pEngine)
{
	m_engine = pEngine;
...
SaveState::~SaveState()
{
	cout << "delete SaveState!" << endl;
}

void SaveState::virtSetupBackgroundBuffer()
{
...
void SaveState::virtMouseDown(int iButton, int iX, int iY)
{
	...
	if (iButton == SDL_BUTTON_LEFT)
	{
		if (iX > 680 && iX < 1220 && iY > 70 && iY < 250) {
			...
			//SimpleImage remind = ImageManager::loadImage("images/remind.png", true);
			//remind.renderImageWithMask(m_engine->getBackgroundSurface(), 0, 0, 550, 300,
			//	remind.getWidth(), remind.getHeight());
...
int SaveState::virtInitialiseObjects()
{
	return 0;
}

void SaveState::virtDrawStringsOnTop()
{
	...
	sprintf(buf1, (char*)line1.c_str());
	sprintf(buf5, (char*)line2.c_str());
	sprintf(buf7, (char*)line3.c_str());
...
void SaveState::virtKeyDown(int ikeyDown)
{
...
void SaveState::saveGame(int id)
{
	ifstream fin;
	ofstream fout;
	int a, b, c, d, e, f, g, h, i, j, k, l, m, n, o, level;
	string str1, str2, str3, str4, str5;
	fin.open("resources/saveInfo.txt", ios::in);
	if (id == 1)	fout.open("resources/Archive1.txt", ios::out);
	else if (id == 2)	fout.open("resources/Archive2.txt", ios::out);
	else if (id == 3)	fout.open("resources/Archive3.txt", ios::out);
	...
	fin >> a >> level >> b >> c >> d >> e >> f >> g >> h >> i >> j >> k >> l >> m >> n >> o;
	fin >> str1 >> str2 >> str3 >> str4 >> str5;
	fout << a << " " << level << " " << b << " " << c << " " << d << " " << e << " " << f << " " << g << " " << h << " " << i << " " << j << " " << k << " " << l << " " << m << " " << n << " " << o
...
void SaveState::loadInfo(int id)
{
	ifstream infoFile;
	string line;
	string str1, str2, str3, str4, str5;
	...
	{
		mapId[id] = -1;
		if (id == 0)	line1 = "No Data";
		else if (id == 1)	line2 = "No Data";
		else if (id == 2)	line3 = "No Data";
	...
	else 
	{
		...
		if (id == 0)	line1 = str1 + " " + str2 + " " + str3 + " " + str4 + " " + str5;
		else if (id == 1)	line2 = str1 + " " + str2 + " " + str3 + " " + str4 + " " + str5;
		else if (id == 2)	line3 = str1 + " " + str2 + " " + str3 + " " + str4 + " " + str5;
	}
...
void SaveState::copyAllBackgroundBuffer()
{
